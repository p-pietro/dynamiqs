...
def test_sepropagator_parallel_cpu_devices():
    ...
    script = textwrap.dedent(
        ...
        def main():
            ...
            result = dq.sepropagator(H, tsave, options=options)
            result.propagators.block_until_ready()
            devices = result.propagators.devices()
            if len(devices) != 2:
